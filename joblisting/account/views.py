...
def login(request):
    if request.method =='POST':
        ...
        if user is not None:
            ...
            error_message = messages.warning(request,"Email or Password didn't match")
    ...
    if request.method == 'POST':
        ...
        if (password1 == password2):
            if User.objects.filter(email=email).exists():
                messages.warning(request,'Email already in use, please login with your email address')
            else:
